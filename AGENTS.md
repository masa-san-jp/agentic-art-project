...
- 正本を確認できない旧要約は`summary.md`として保存し、statusを`blocked-missing-canonical`、`canonical_plan_available: false`にする。`plan.md`を置かず、制作可能として一覧表示しない。
- 正規planは`canonical-plan-projection/v1`、`AUTOMATIC_PLAN`、`body_transform: none`、Production repository/commit、run ID、canonical SHA-256をmetadataに持つ。
